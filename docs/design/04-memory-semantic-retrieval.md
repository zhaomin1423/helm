...
**✓ 已实现（基础）**。`IndexedEmbeddingStore` SPI（解耦自 `InMemoryEmbeddingStore`）、`SemanticMemoryStore` 装饰器、embedding 维度校验、`FakeEmbeddingProvider`。仍待实现：真实向量库 adapter（pgvector/Weaviate 等）。
