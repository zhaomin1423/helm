...
/**
 * Auto-configures Helm runtime beans from application beans and optionally mounts the HTTP routes. The starter only
 * assembles; no runtime behavior lives here. Core Helm modules stay Spring-free.
 */
@AutoConfiguration
...
public class HelmAutoConfiguration {
    ...
    public AgentRuntime helmAgentRuntime(
            List<AgentDefinition> agents, List<ModelProvider> providers, RuntimeStore store) {
    ...
    public ServletRegistrationBean<HelmHttpServlet> helmHttpServlet(
            AgentRuntime agentRuntime, WorkflowRuntime workflowRuntime, HelmProperties properties) {
